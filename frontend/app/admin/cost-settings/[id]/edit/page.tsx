...
import { updateCostSetting, useCostSettings } from "@/hooks/useCostSettings"
import { useDeviceTypes } from "@/hooks/useDeviceTypes"
import { useProblems } from "@/hooks/useProblems"
import { Breadcrumb } from "@/components/breadcrumb"

export default function AdminEditCostSettingPage() {
  ...
  const current = costSettings.find((c) => c.id === id)

  const [formData, setFormData] = useState({
    device_type_id: current?.device_type_id.toString() || "",
    problem_id: current?.problem_id.toString() || "",
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const message = e instanceof Error ? e.message : "Failed to update cost setting"
      setError(message)
    } finally {
      setLoading(false)
    }
    ...
    <SidebarProvider>
      ...
      <SidebarInset>
        ...
        <div className="flex-1 space-y-4 p-4 md:p-6">
          ...
          {/* Header */}
          <div>
            <h1 className="text-3xl font-bold tracking-tight">Edit Cost Setting</h1>
            <p className="text-muted-foreground text-sm mt-1">
              Update repair cost details
            </p>
          </div>
          ...
          <div className="max-w-2xl">
            <Card>
              <CardHeader>
                <CardTitle>Cost Setting #{id}</CardTitle>
                <CardDescription>
                  Update the repair cost for this device type and problem combination
                </CardDescription>
              </CardHeader>
              <CardContent>
