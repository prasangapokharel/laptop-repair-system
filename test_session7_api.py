...
def get_auth_token(phone: str, password: str):
...
def test_device_models(token: str) -> bool:
    ...
    if brands_resp.status_code == 200 and types_resp.status_code == 200:
        ...
        if brands and types:
            ...
            print("   ⚠️  No brands or types available to create model")
            return True
...
def main():
    print("=" * 70)
    print(" SESSION 7 - COMPREHENSIVE FEATURE TESTING")
    print("=" * 70)
    
    # Test with admin token
    print("\n[AUTH] Authenticating with admin credentials...")
    ...
    # Test 2: Payment auto-calculation
    if orders:
        if not test_payment_calculation(admin_token, orders):
            all_passed = False
    else:
        print("\n⚠️  Skipping payment tests (no orders available)")
    
    # Test 3-5: Device management
    if not test_device_types(admin_token):
        all_passed = False
    
    if not test_device_brands(admin_token):
        all_passed = False
    
    if not test_device_models(admin_token):
        all_passed = False
